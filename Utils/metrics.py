...
def eval_metrics(true, pred, labels=[1,0], domain='book'):
    eval_dict = dict()
    TP, FN, FP, TN = confusion_matrix(true, pred, labels=labels).ravel()
    
    try:
        eval_dict['ACC'] = accuracy_score(true, pred)
        eval_dict['Precision'] = precision_score(true, pred)
        eval_dict['Recall'] = recall_score(true, pred)
        eval_dict['F1-score'] = f1_score(true,pred)
        eval_dict['FPR'] = FP / (FP + TN)
        eval_dict['TP'] = TP
        eval_dict['FN'] = FN
        eval_dict['FP'] = FP
        eval_dict['TN'] = TN
    ...
    print(f'Domain {domain} performance')
